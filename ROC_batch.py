...
from loses import f1score_with_channel, precision_with_channel, recall_with_channel
...
def evaluate(config):
    ...
    def wrapped_partial(func,*args,**kwargs):
        ...
        partial_func.__name__ = f'{func.__name__}-{kwargs["thresh_p"]:.03}-{kwargs["channel"]}'
        return partial_func
    ...
    metrics = [wrapped_partial(precision_with_channel,thresh_p=thresh_p,channel = channel) for thresh_p in np.linspace(0,1,21) for channel in [None, 0, 1, 2, 3]]
    metrics = metrics + [wrapped_partial(recall_with_channel,thresh_p=thresh_p,channel = channel) for thresh_p in np.linspace(0,1,21) for channel in [None, 0, 1, 2, 3]]
    metrics = metrics + [wrapped_partial(f1score_with_channel,thresh_p=thresh_p,channel = channel) for thresh_p in np.linspace(0,1,21) for channel in [None, 0, 1, 2, 3]]
    if multi == True:
