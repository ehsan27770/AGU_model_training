...
def evaluate(config):
    ...
    ####################################################################### compile
    metrics = [loss_ce, loss_dice, loss_jaccard, loss_focalc, accuracy, precision, recall, f1score]
    metrics = metrics + [wrapped_partial(f1score_with_channel,channel = channel) for channel in [0, 1, 2, 3]]
    metrics = metrics + [wrapped_partial(precision_with_channel,channel = channel) for channel in [0, 1, 2, 3]]
    metrics = metrics + [wrapped_partial(recall_with_channel,channel = channel) for channel in [0, 1, 2, 3]]
